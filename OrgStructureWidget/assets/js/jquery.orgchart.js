(function($) {
    ...
    function Node(data){
        ...
        this.render = function(opts){
            ...
            if(childLength > 0){
                var downLineTable = "<table cellpadding='0' cellspacing='0' border='0'><tr class='lines x'><td class='line left-org half'></td><td class='line right half'></td></table>";
                mainTable += "<tr class='lines'><td colspan='"+childLength*2+"'>"+downLineTable+'</td></tr>';

                var linesCols = '';
                for(var i=0;i<childLength;i++){
                    if(childLength==1){
                        linesCols += "<td class='line left-org half'></td>";    // keep vertical lines aligned if there's only 1 child
                    }
                    else if(i==0){
                        linesCols += "<td class='line left-org'></td>";     // the first cell doesn't have a line in the top
                    }
                    else{
                        linesCols += "<td class='line left-org top'></td>";
                    }
        ...
        this.formatNode = function(opts){
            ...
            if(typeof data.name !== 'undefined'){
                nameString = '<a href="#">'+self.data.name+'</a>';
            }
            ...
            return "<div class='node' node-id='"+this.data.id+"'>"+nameString+"</div>";
        }
    }
